...
<body>
    ...
    <div class="container">
                ...
                <div class="dn-selector" id="dn-selector">
                    ...
                    <div class="dn-dropdown" id="dn-dropdown">
                        <div class="dn-dropdown-header">
                            ...
                            <div class="dn-dropdown-actions"><a onclick="selectAllDN()">Vse</a><a onclick="deselectAllDN()">Poèisti</a></div>
                        </div>
        ...
        <div class="kpi-row">
            ...
            <div class="kpi-card"><div class="kpi-value" id="kpi-avg">--</div><div class="kpi-label">Povpreèje</div><div class="kpi-change" id="kpi-avg-change">--</div></div>
    ...
    <script>
        ...
        function processData(){
            ...
            const locMap={};
            rawData.forEach(r=>{const l=r['Person Primary Location']||'Neznano';if(!locMap[l])locMap[l]={id:l.toLowerCase().replace(/\s+/g,'-'),name:l,color:getColor(Object.keys(locMap).length)};});
            ...
            entries=rawData.map((r,i)=>{
                ...
                const locName=r['Person Primary Location']||'Neznano';
                const loc=lokacije.find(l=>l.name===locName)||{id:'neznano',name:'Neznano',color:'#94a3b8'};
                let checkin=null,checkout=null;
                if(r['Actual Start Time']){const p=r['Actual Start Time'].split(':');if(p.length>=2){checkin=new Date();checkin.setHours(parseInt(p[0])||0,parseInt(p[1])||0,0,0);}}
                if(r['Actual End Time']){const p=r['Actual End Time'].split(':');if(p.length>=2){checkout=new Date();checkout.setHours(parseInt(p[0])||0,parseInt(p[1])||0,0,0);}}
                return{id:i+1,employee:{id:r['Person ID']||i,name:r['Person Name']||'Neznano',location:loc.id,costCenter:r['Person Cost Center ID Name']||'Neznano',employmentType:r['Person Group Name']||'Neznano',initials:getInitials(r['Person Name'])},dn,workCenter:r['Work Center ID Name']||'',checkin,checkout,datePosting:r['Date Posting']?new Date(r['Date Posting']):new Date(),duration:parseFloat(r['M Hours'])||0};
            });
